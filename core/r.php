...
class RedBean_OODB extends RedBean_Observable {
	...
	public function convertToBeans($type, $rows) {
		...
		foreach($rows as $row) {
			$id = $row['id'];
			$this->stash[$index] = $row;
			$collection[ $index ] = $this->load( $type, $id );
			$index++;
		}
		$this->stash = NULL;
		return $collection;
...
class RedBean_Facade {
	...
	public static function tagged( $beanType, $tagList, $sql=null, $values=array() ) {
		if ($tagList!==false && !is_array($tagList)) $tags = explode( ",", (string)$tagList); else $tags=$tagList;
		$collection = array();
		foreach($tags as $tag) {
			$retrieved = array();
			$tag = RedBean_Facade::findOne('tag',' title = ? ', array($tag));
			if ($tag) $retrieved = RedBean_Facade::related($tag, $beanType, $sql, $values);
